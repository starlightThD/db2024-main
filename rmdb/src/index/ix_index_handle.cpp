...
void IxIndexHandle::insert_into_parent(IxNodeHandle *old_node, const char *key, IxNodeHandle *new_node,
    ...
    if (old_node->is_root_page()) {
        ...
        root->insert_pair(1, new_node->get_key(0), Rid{new_node->get_page_no(), -1});
    ...
    parent->insert_pair(old_pos + 1, new_node->get_key(0), Rid{new_node->get_page_no(), -1});
...
page_id_t IxIndexHandle::insert_entry(const char *key, const Rid &value, Transaction *transaction) {
    ...
    if (leaf == nullptr) {
        throw InternalError("Index root page not found");
    }
...
bool IxIndexHandle::delete_entry(const char *key, Transaction *transaction) {
    ...
    if (removed && leaf->get_size() > 0 && old_pos == 0) {
        maintain_parent(leaf);
    }
...
Iid IxIndexHandle::lower_bound(const char *key) {
    ...
    Iid iid{leaf->get_page_no(), leaf->lower_bound(key)};
...
Iid IxIndexHandle::upper_bound(const char *key) {
    ...
    Iid iid{leaf->get_page_no(), leaf->upper_bound(key)};
    while (iid.slot_no == leaf->get_size() && leaf->get_next_leaf() != IX_LEAF_HEADER_PAGE) {
        page_id_t next_page = leaf->get_next_leaf();
        buffer_pool_manager_->unpin_page(leaf->get_page_id(), false);
        leaf = fetch_node(next_page);
...
Iid IxIndexHandle::leaf_begin() const {
    return {.page_no = file_hdr_->first_leaf_, .slot_no = 0};
}
...
void IxIndexHandle::maintain_parent(IxNodeHandle *node) {
    ...
    while (curr->get_parent_page_no() != IX_NO_PAGE) {
        ...
        if (curr->get_size() == 0 ||
            memcmp(parent->get_key(rank), curr->get_key(0), file_hdr_->col_tot_len_) == 0) {
